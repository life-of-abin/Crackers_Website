    ...
    <div className="space-y-6 selection:bg-[#6D3FD6] selection:text-white">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 sm:p-6 rounded-3xl border border-slate-200 shadow-sm gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-1">Manage fulfillment status and generate invoice</p>
        </div>
        <button
          onClick={() => window.open(`/admin/orders/${order.id}/invoice`, '_blank')}
          className="bg-[#6D3FD6] hover:bg-[#5B21B6] text-white px-6 py-2.5 rounded-xl font-black text-xs tracking-wide transition-all shadow-md shadow-purple-200 flex items-center gap-2 w-full sm:w-auto justify-center cursor-pointer"
        >
